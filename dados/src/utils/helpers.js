...
async function getLidFromJidCached(bender, jid) {
  if (!isValidJid(jid)) {
...
async function convertIdsToLid(bender, ids) {
  ...
  for (let i = 0; i < ids.length; i += batchSize) {
    ...
    const batchPromises = batch.map(id => getLidFromJidCached(bender, id));
...
async function normalizeUserId(bender, userId) {
  ...
  if (isValidJid(userId)) {
    return await getLidFromJidCached(bender, userId);
  }
